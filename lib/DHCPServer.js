    ...
    return new Promise((resolve, reject) => {
      ...
      this._socket.on('error', (e) => {
        ...
        if (this.started) {
          ...
          debug(`\tError = ${e}`);
          e.message = `Impossible to create socket "${this._config.serverIp}:${this._config.serverPort}": ${e.message}`;
          reject(e);
        }
      });
      this._socket.once('listening', () => {
        ...
        try { // Then sniff interface for requests
          ...
          debug(`\tError = ${e}`);
          e.message = `Impossible to sniff in interface "${this._config.interface}": ${e.message}`;
          reject(e);
        }
      });
    });
